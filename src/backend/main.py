...
class room:
    def __init__(self, width, height, anchorTopLeftX, anchorTopLeftY, text):
        self.width = width
        self.height = height
        self.anchorTopLeftX = anchorTopLeftX
        self.anchorTopLeftY = anchorTopLeftY
        self.text = text
...
class building:
    ...
    def write_to_json(self):
        ...
        data["planBoundary"] = {}
        data["planBoundary"]["x"] = self.x
        data["planBoundary"]["y"] = self.y   

        data["rooms"] = []
    
        for r in (self.topRooms + self.leftRooms + self.rightRooms + self.bottomRooms + self.middleRooms):
            d = {}
            d["id"] = r.id
            d["width"] = r.x
            d["height"] = r.y
            d["anchorTopLeftX"] = r.posTopLeft[0]
            d["anchorTopLeftY"] = self.y - r.posTopLeft[1]
            d["type"] = r.id
            data["rooms"].append(d)
